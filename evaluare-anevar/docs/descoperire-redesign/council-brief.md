...
- **Dimensiunea API (important).** Pipeline-ul = scraping HTTP direct (`fetch_html`) + parsare HTML/`__NEXT_DATA__` + LLM (Claude) pe textul scrapuit. **Perplexity NU e în pipeline.** **Verificat independent de TREI ori (2026-06-07, eșantion mic dar concordant).** Perplexity (Sonar) face **web-search real** — *citările* lui sunt pagini reale de listare/căutare — DAR **nu deschide paginile individuale de anunț**, deci nu poate da date per-anunț de încredere; comportamentul depinde de prompt:
  ...
  **Concluzie (verificată, NU prescriptivă):** un API de tip search e util cel mult ca **strat de DESCOPERIRE a surselor** (ne-a găsit chiar portaluri în plus — vezi mai jos), **nu** ca strat de găsire/extracție de anunțuri comparabile. Pentru **găsirea comparabilelor**, scraping-ul nostru funcționează deja end-to-end; un API de search **nu** îl poate înlocui. Un LLM e sigur la **extracția din textul pe care i-l dăm noi**, nu la regăsirea/citirea anunțurilor.
...
**Q5 (NOU) — Ce API e potrivit (dacă vreunul) pentru cele 3 site-uri.** Pentru imobiliare/storia/olx, știind că azi facem **scraping HTTP direct + extracție LLM pe text**: premisa noastră de lucru (verificată independent de **trei** ori, eșantion mic — de contestat sau confirmat) e că un API de tip search **NU** poate da datele structurate per-anunț — Perplexity face web-search real (citările lui sunt pagini reale) **dar nu deschide paginile individuale**, deci ori întoarce gol, ori **fabrică** deep-link-uri (3/4 = 404), ori (când e onest) refuză. Întrebări:
- **Unde** ar aduce valoare un API în pipeline — la *search/discovery a surselor* (unde Perplexity chiar a găsit portaluri în plus), la *fetch*, sau **doar la extracția din textul scrapuit**? Sau scraping-ul direct rămâne abordarea corectă end-to-end?
- *(Tiering / cost — idee de produs de evaluat)* Are sens să **diferențiem modulul după soluția de găsire**, cu paliere de cost/consum? Ex.: (i) **scraping** = gratis, inclus sub consum rezonabil; (ii) **API plătit** (~0.5 €/call sau inclus într-un pachet mai mare) pentru acoperire/robustețe mai mare; (iii) bundle. Cum ar arăta un model viabil și ce palier merită pentru ce categorie/volum?
Vă rugăm tratați premisa „search-API nu poate da datele structurate per-anunț" ca ipoteza-cheie de validat sau respins.
