...
```
căutare pe portal → scrape HTML (fetch_html, HTTP direct) → parsare anunț (HTML / __NEXT_DATA__)
  → extracție atribute (LLM, Claude) → scor determinist → rank
```
> Model mental cheie: **scraperul GĂSEȘTE anunțurile; LLM-ul doar le PROCESEAZĂ** (extrage atribute din text). Găsirea ≠ procesarea — vezi §3 „Dimensiunea API".
...
- **Dimensiunea API (important).** Pipeline-ul = scraping HTTP direct (`fetch_html`) + parsare HTML/`__NEXT_DATA__` + LLM (Claude) pe textul scrapuit. **Perplexity NU e în pipeline.** **Verificat independent de TREI ori (2026-06-07, eșantion mic dar concordant).** Perplexity (Sonar) face **web-search real** — *citările* lui sunt pagini reale de listare/căutare — DAR **nu deschide paginile individuale de anunț**, deci nu poate da date per-anunț de încredere; comportamentul depinde de prompt:
  - (A) *extracție dintr-un URL dat* → a întors **`{}`** (nu accesează pagina). **Re-confirmat pe 2 URL-uri storia** → „nu pot accesa" + citări complet irelevante (a făcut keyword-match pe slug — „ATV/vacanță" — **nu a deschis anunțul**). Insight: Perplexity **nu deschide URL-ul dat**; face o căutare nouă pe cuvinte. Inaccesibilitatea e **portal-agnostică** (imobiliare ȘI storia).
  - (B) *„dă 4 anunțuri cu URL"* (forțat) → a **fabricat** deep-link-uri: **3/4 = 404** (inexistente), 1/4 real, cu prețuri plauzibile lipite pe URL-uri false.
  - (C) *„dă 6 anunțuri cu preț/suprafață/an"* (re-test mai mare) → a fost **onest și a refuzat**: „pot vedea doar rezumate/listări agregate, nu pagina fiecărui anunț".
  - (D) **TEST DECISIV** — *„găsește comparabile pentru ACEST subiect (în format app), caută pe tot"* (subiect real: casă Breaza ~210mp, teren 400mp, an 2005, centrală gaz) → Perplexity **a refuzat din nou** (vede doar landing-page-uri de portal). **Head-to-head pe ACELAȘI subiect: scraperul nostru (storia) a găsit 8 comparabile reale, rankate (80/65/65/58/51/48%…), 4/4 URL-uri verificate valide (200). Perplexity = 0.**
  **⚠️ CORECȚIE (panel OpenRouter, 12 modele, 2026-06-07) — NU generaliza eșecul Perplexity:**
  - **Extracția** (din textul scrapuit de noi): **9/9 modele testate au extras corect toate atributele** (scor 12/12 pe 2 anunțuri) — inclusiv open ieftine (`llama-3.3-70b` la **$0.10/$0.32 per M token**, ~12× mai ieftin la output decât `claude-3.5-haiku` pe care-l folosim). Constatare: pe acest eșantion, calitatea e egală la costuri foarte diferite. *(Întrebarea „merită schimbat modelul" e deschisă la Q5.)*
  - **Discovery web — depinde de backend-ul de search, NU e imposibil:** `perplexity/sonar` = slab (0/1 reale), DAR **`openai/gpt-4o-mini:online` și `google/gemini-2.5-flash:online` (search Exa) au găsit anunțuri REALE individuale (6/6 și 4/4 verificate 200), inclusiv de pe `imoradar24.ro` — sursă pe care NU o scrapuim.** Deci un search-API BUN (Exa `:online`) **poate** servi ca strat de descoperire de candidați. **Test de complementaritate (cerut explicit alte surse decât imobiliare/storia):** `:online` a întors **consistent imoradar24.ro** (5–6 reale/model) + un publi24 (504, greu de fetch). Deci complementaritatea reală e **concentrată pe imoradar24** — care e **AGREGATOR** (re-listează din portaluri, inclusiv cele pe care deja le avem → necesită **dedup**); nu a adus direct agenții locale/hyperlocal. Practic: un parser dedicat imoradar24 ar da același efect, mai controlat.
  **Constatare empirică (NU concluzie de produs):** modelul mental real = **scraperul GĂSEȘTE anunțurile, LLM-ul doar le PROCESEAZĂ (extrage din text)**. Moat-ul practic = parsarea structurată (storia `__NEXT_DATA__`), care găsește deja comparabile end-to-end. Extracția dă rezultate egale pe mai multe modele la costuri diferite. Un search-API bun (Exa `:online`) a întors anunțuri reale; Sonar nu. **Rămâne DESCHIS** (Q5/Q7) dacă merită adăugat un strat de discovery hibrid față de status quo (scraper + un LLM).
- **Alte site-uri RO:** **VDI.ro și Imoradar24.ro acoperă COMERCIAL + INDUSTRIAL** — exact golul nostru pe #3 (industrial) și #5 (special/comercial), pe care imobiliare/storia nu le acoperă bine. Plus HomeZZ.ro, CompariImobiliare.ro (agregator), Publi24.ro, anuntul.ro. **Bonus:** căutarea Perplexity a scos la iveală și `phimob.ro`, `romimo.ro`, `mervani.ro`, `imopedia.ro` și un portal **hyperlocal** — `imobiliarebreaza.ro` (relevant: agențiile locale listează exact zona). **Fiecare cere parser propriu** → modelarea #3/#5 depinde și de adăugarea acestor surse. (Aici un API de search ar putea fi util ca *strat de descoperire a surselor*, vezi „Dimensiunea API".)
...
**Q5 (NOU) — Ce API/model pentru ce strat.** *(Datele empirice care motivează întrebările sunt în §3 „Dimensiunea API" — aici doar întrebările, ca să nu repetăm cifrele.)*
- **Extracție:** dat fiind că mai multe modele extrag la fel de bine la costuri foarte diferite (§3), **ce model alegem** și **merită să trecem de pe Claude pe unul mai ieftin**? Cu ce riscuri pe text dificil / categorii noi?
- **Descoperire web:** rezultate **mixte între backend-uri de search** (§3: Sonar 0/1 reale vs. Exa `:online` 6/6 și 4/4). **Merită** un strat de discovery via search-API (Exa) ca hibrid (search găsește candidați → parserul nostru extrage), **dat fiind că scraperul nostru găsește deja comparabile end-to-end**? Sau un parser dedicat pe agregatorul imoradar24 e mai controlat? Riscuri (fiabilitate, drift, dedup, cost)?
- **Tiering / cost (idee de produs):** are sens să diferențiem modulul după soluția de găsire, cu paliere — (i) scraping gratis sub consum rezonabil; (ii) search-API plătit (~0.5 €/call sau în pachet); (iii) bundle? Ce model viabil, ce palier pentru ce volum/categorie?
