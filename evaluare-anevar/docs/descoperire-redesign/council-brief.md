...
- **Dimensiunea API (important).** Pipeline-ul = scraping HTTP direct (`fetch_html`) + parsare HTML/`__NEXT_DATA__` + LLM (Claude) pe textul scrapuit. **Perplexity NU e în pipeline.** **Verificat independent de TREI ori (2026-06-07, eșantion mic dar concordant).** Perplexity (Sonar) face **web-search real** — *citările* lui sunt pagini reale de listare/căutare — DAR **nu deschide paginile individuale de anunț**, deci nu poate da date per-anunț de încredere; comportamentul depinde de prompt:
  ...
  **⚠️ CORECȚIE (panel OpenRouter, 12 modele, 2026-06-07) — NU generaliza eșecul Perplexity:**
  - **Extracția = commodity:** dat textul scrapuit de noi, **9/9 modele testate au extras perfect (12/12)** atributele — inclusiv open ieftine (`llama-3.3-70b` la **$0.10/$0.32 per M**, ~12× mai ieftin la output decât `claude-3.5-haiku` pe care-l folosim). Concluzie: extracția e **interschimbabilă și ieftină** (gpt-4o-mini / deepseek / llama o fac la fel ca Claude). **Putem ieftini cu ~85%.**
  - **Discovery web — depinde de backend-ul de search, NU e imposibil:** `perplexity/sonar` = slab (0/1 reale), DAR **`openai/gpt-4o-mini:online` și `google/gemini-2.5-flash:online` (search Exa) au găsit anunțuri REALE individuale (6/6 și 4/4 verificate 200), inclusiv de pe `imoradar24.ro` — sursă pe care NU o scrapuim.** Deci un search-API BUN (Exa `:online`) **poate** servi ca strat de descoperire de candidați.
  **Concluzie generală (verificată):** moat-ul = **parsarea structurată** (storia `__NEXT_DATA__`); **extracția** = ieftină/interschimbabilă; **descoperirea** poate fi extinsă cu un search-API bun (Exa `:online`, NU Sonar) ca să prindem surse pe care nu le avem (hibrid: `:online` găsește candidați larg → parserul nostru extrage).
...
**Q5 (NOU) — Ce API/model pentru ce strat.** Date empirice (panel OpenRouter 12 modele + Perplexity, 2026-06-07):
- **Extracție = commodity ieftin:** 9/9 modele au extras perfect din textul scrapuit (gpt-4o-mini/deepseek/llama-3.3-70b la fel ca Claude, ~5–12× mai ieftin). Întrebare: ce model alegem pentru extracție și **merită să trecem de pe Claude pe unul mai ieftin** (ex. llama-3.3-70b $0.10/$0.32)? Riscuri de fiabilitate pe text dificil / categorii noi?
- **Descoperire web — funcționează cu search-ul potrivit:** `perplexity/sonar` = slab (nu găsește), DAR `:online` cu **Exa** (gpt-4o-mini:online, gemini-flash:online) **a găsit anunțuri reale verificate (6/6, 4/4), inclusiv din surse pe care NU le scrapuim (imoradar24)**. Întrebare: integrăm un strat de discovery via search-API (Exa) ca să prindem surse fără parser propriu — **hibrid** (search găsește candidați → parserul nostru extrage de pe portalurile suportate)? Cu ce riscuri (fiabilitate, drift, cost)?
- *(Tiering / cost — idee de produs)* Diferențiem modulul după soluția de găsire, cu paliere: (i) **scraping** = gratis sub consum rezonabil; (ii) **search-API plătit** (~0.5 €/call sau în pachet) pentru acoperire mai largă; (iii) bundle. Ce model viabil și ce palier pentru ce volum/categorie?
