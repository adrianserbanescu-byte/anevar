...
class Comparable(BaseModel):
    ...
    pret: Decimal = Field(gt=0)         # pret pozitiv (un comparabil cu pret 0/negativ e invalid)
    suprafata: Decimal = Field(gt=0)    # mp, numitorul pretului unitar -> >0 (altfel DivisionByZero -> 500)
...
class LandComparable(BaseModel):
    ...
    pret_mp: Decimal = Field(gt=0)      # €/mp pozitiv
    suprafata: Decimal = Field(gt=0)    # mp > 0
...
class RentComparable(BaseModel):
    ...
    suprafata: Decimal = Field(gt=0)   # mp > 0 (consistenta cu Comparable/LandComparable)
    localizare: str | None = None
    data: str | None = None
    adjustments: list[Adjustment] = Field(default_factory=list)
