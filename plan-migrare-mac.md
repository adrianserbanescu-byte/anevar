...
4. **Hooks — rescriere pentru macOS** (echivalentele celor 5 globale + 2 de proiect):
   ...
   - proiect (`~/Projects/anevar/.claude/settings.local.json`, UserPromptSubmit): DOAR `python3` pe `live-up.py` portat (Faza 1.2). Hook-ul `mailbox.py` NU se recreează — sistemul A–F e desființat (2026-06-11).
5. **claude-mem:**
   ```bash
...
**Pe Mac:**
5. `git pull` în `~/Projects/anevar`.
6. Transcripturi: conținutul folderului Windows → `~/.claude/projects/-Users-<user>-Projects-anevar/` (numele NOU encodat — nu păstra numele `C--Users-adyse-anevar`!). Inclusiv `memory/`. (Worktree-uri: nu mai există — D3 anulată.)
